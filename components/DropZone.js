...
const Dropzone = ({ className, }) => {
    ...
    async function action() {
        try {
            ...
            const endpoint = "https://api.cloudinary.com/v1_1/dqssz4rkk/image/upload"
            ...
            setUploadedImageData([...uploadedImageData, data]);
        } catch (error) {
    ...
    return (
        <form action={action}>
            <div
                ...
                className='border border-[#0b0804] p-4 rounded-lg'
            >
            ...
            <section className='mt-10'>
                <div className='flex gap-4'>
                    ...
                    <button
                        type='button'
                        onClick={removeAll}
                        className='mt-1 rounded-md border border-[#0b0804] px-3 text-[12px] font-bold uppercase tracking-wider text-stone-500 transition-colors hover:bg-rose-400 hover:text-white'
                    >
                        Remove all files
                    </button>
                    <button
                        type='submit'
                        className='ml-auto mt-1 rounded-md border border-[#0b0804] px-3 text-[12px] font-bold uppercase tracking-wider text-stone-500 transition-colors hover:bg-gray-200 hover:text-[#0b0804]'
                    >
                        Upload
                    </button>
                </div>
            ...
            <ul className="p-5 md:p-10">
                <h1 className='mt-4 md:mt-8 px-3 md:px-6 font-bold mb-3'>My Uploads</h1>
                <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                    {uploadedImageData.map((image, index) => (
                        <li key={index}>
                            <Image
                                src={image.url}
                                alt="uploaded"
